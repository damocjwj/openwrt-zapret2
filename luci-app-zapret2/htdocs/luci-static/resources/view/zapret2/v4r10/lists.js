...
function editList(item) {
	...
	return (existing ? api.listGet(existing.id, existing.type) : Promise.resolve({ id: '', type: 'domain', content: '' })).then(function(data) {
		...
		ui.showModal(existing ? _('Edit local list') : _('Create local list'), [
			...
			E('p', { class: 'cbi-value-description' }, _('Domain lists accept domains and ^exact.domain. IP lists accept IPv4, IPv6 and CIDR. Empty lines and # comments are allowed.')),
			referenced ? E('p', { class: 'alert-message notice' }, _('Referenced lists may be edited, but their ID and type are locked until all Profile references are removed.')) : E([]),
			content,
			E('div', { class: 'right' }, [ E('button', { class: 'btn', click: ui.hideModal }, _('Cancel')), ' ', E('button', { class: 'btn cbi-button-positive', click: function() {
				...
				if (bytes(text) > +(limits.file_bytes || 1048576)) { zui.notifyError(new Error(_('The list exceeds the backend file-size limit.'))); return; }
...
function clearAuto(item) {
	zui.confirm(_('Clear automatic hostlist'), _('Atomically clear the engine-managed automatic hostlist for %s?').format(item.id), _('Clear'), function() { return api.listClear(item.id).then(refresh); });
}
...
function table(type, title) {
	...
	return E('div', { class: 'cbi-section' }, [ E('h3', {}, title), E('table', { class: 'table' }, [
		E('tr', { class: 'tr table-titles' }, [ E('th', { class: 'th' }, _('ID')), E('th', { class: 'th' }, _('Entries')), E('th', { class: 'th' }, _('Size')), E('th', { class: 'th' }, _('Referenced by')), E('th', { class: 'th' }, _('Actions')) ])
	].concat(items.length ? items.map(function(item) {
		var actions = [ E('button', { class: 'btn', click: ui.createHandlerFn(null, exportList, item) }, _('Export')) ];
		if (type === 'auto_domain') actions.push(' ', E('button', { class: 'btn cbi-button-negative', click: ui.createHandlerFn(null, clearAuto, item) }, _('Clear')));
		else {
			actions.unshift(E('button', { class: 'btn cbi-button-action', click: ui.createHandlerFn(null, editList, item) }, _('Edit')), ' ');
		...
		return E('tr', { class: 'tr' }, [ E('td', { class: 'td', 'data-title': _('ID') }, item.id), E('td', { class: 'td', 'data-title': _('Entries') }, String(item.entries || 0)), E('td', { class: 'td', 'data-title': _('Size') }, '%1024.2mB'.format(item.size || 0)), E('td', { class: 'td', 'data-title': _('Referenced by') }, (item.profiles || []).join(', ') || '-'), E('td', { class: 'td', 'data-title': _('Actions') }, actions) ]);
...
function renderManager() {
	return E([], [
		...
		E('div', { class: 'cbi-page-actions' }, [ E('button', { class: 'btn cbi-button-add', disabled: index.filter(function(item) { return item.type !== 'auto_domain'; }).length >= +(limits.count || 32), click: ui.createHandlerFn(null, editList, null) }, _('Create list')), ' ', E('button', { class: 'btn cbi-button-reload', click: ui.createHandlerFn(null, refresh) }, _('Refresh')) ])
	]);
}
...
return view.extend({
	...
	render: function(data) {
		if (data[0].error || data[1].error)
			return E('div', { class: 'cbi-map' }, [ E('h2', {}, _('Zapret2 local lists')), E('div', { class: 'alert-message error' }, zui.errorText(data[0].error || data[1].error)) ]);
		model.setInfo(data[0]); limits = data[0].list_limits || {}; index = data[1].lists || [];
		return E('div', { class: 'cbi-map' }, [ E('h2', {}, _('Zapret2 local lists')), E('div', { class: 'cbi-map-descr' }, _('Static lists are atomically validated and replaced. Automatic lists are owned by nfqws2 and can only be viewed, exported, or cleared.')), E('div', { id: 'zapret2-list-content' }, renderManager()) ]);
	},
